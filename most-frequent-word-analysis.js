...
}// .toLowerCase takes case sensitivity out of the equation. .split will remove punctuation so it is not counted as words i believe, and sort will sort our words aplhabetically
...
function mostFrequentWord(text) {
    ...
    for (var i = 0; i <= words.length; i++) {
        ...
        if (words[i] in wordFrequencies) {
            ...
            // generates an if statement that says if the value at words[i] is already in the //object wordFrequencies, then increment the value
        }
        else {
            wordFrequencies[words[i]]=1;
            // generates an else statement that says since the value at words[i] was NOT in the //object, add it to the wordFrequencies object
            //**bracket notation**
        }
    }
    var currentMaxKey = Object.keys(wordFrequencies)[0];
    // new local variable currentMaxKey that has the value of an array of the contents of wordFrequencies, starting at the first item in the array
    var currentMaxCount = wordFrequencies[currentMaxKey];
    // new local variable currentMaxCount has the value of the object wordFrequencies at the index defined by the previous variable[currentMaxKey]
